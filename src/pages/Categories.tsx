...
const Categories = () => {
  ...
  const categories = useSelector((state: RootState) => state.categories, (prev, next) => prev === next);
              ...
              <tbody>
                {categories?.map((category: ICategory) => (
                  <tr key={category.id}>
                    ...
                    <td className="px-5 py-4 text-sm bg-white border-b border-gray-200">
                      <span
                        className={`relative inline-block px-3 py-1 font-semibold leading-tight ${category.status ? "text-green-900" : "text-red-900"}`}
                      >
                        <span
                          aria-hidden="true"
                          className={`absolute inset-0 rounded-full opacity-50 ${category.status ? "bg-green-200" : "bg-red-200"}`}
                        ></span>
